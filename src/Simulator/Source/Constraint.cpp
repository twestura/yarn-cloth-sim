...
bool Length::eval(VecXe& xStar, real omega) {
  ...
  for (int i=0; i<y.numSegs(); i++) {
    Vec3e p1 = xStar.block<3,1>(3*i, 0);
    Vec3e p2 = xStar.block<3,1>(3*(i+1), 0);
    ...
    xDelta.block<3,1>(3*i, 0) += omega * p1gradC * p1lambda; // / (i == 0 ? 1.0 : 2.0);
    xDelta.block<3,1>(3*(i+1), 0) += omega * p2gradC * p2lambda; // / (i == y.numSegs()-1 ? 1.0 : 2.0);
  }
  xStar += xDelta;
  return true;
}